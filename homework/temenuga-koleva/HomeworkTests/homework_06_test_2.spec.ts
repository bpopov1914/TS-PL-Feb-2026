...
test.describe('Homework 06 test scenario 2', { tag: '@hw-6-2' }, () => {
    ...
    await test.step('Navigate to PlaywrightPractice Page', async () => {
        ...
        await expect(page, "Verify Page title").toHaveTitle('Automation Testing Practice: PlaywrightPractice');
     });

await test.step('Click on Home link', async () => {
  await page.locator('#role-locators').getByRole('link', { name: 'Home' }).click();
  await expect(page, "Verify Navigation to Home").toHaveURL('https://testautomationpractice.blogspot.com/p/playwrightpractice.html#');
});

await test.step('Verify colored text is red', async () => {
    await expect(page.getByText('colored text', { exact: true })).toHaveCSS('color', 'rgb(255, 0, 0)'); // red
});
...
await test.step('Fill input field with text', async () => {
  ...
  await expect(page.locator('#placeholder-locators').getByPlaceholder('Type your message here...')).toBeEditable();
...
await test.step('Hover over element with title "HyperText Markup Language"', async () => {
    ...
    await expect(page.locator('#title-locators').getByTitle('HyperText Markup Language')).toHaveText('HTML');
   });

await test.step('Verify element with test id "edit-profile-btn" contains text "Profile"', async () => {
    await expect(page.locator('#testid-locators').getByTestId('edit-profile-btn')).toContainText('Profile');
   });
});
});